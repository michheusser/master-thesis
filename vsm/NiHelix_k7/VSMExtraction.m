...
[~, M_ax_rad_simplified ] = SimplifyMH(H_app_ax,M_ax_rad,m_scale_rad,'Zero');
[~, M_rad_ax_simplified ] = SimplifyMH(H_app_rad,M_rad_ax,m_scale_ax,'Zero');

%% Splitting of Simplified Model (Positive Part)
H_app_monotone_ax = 1/mu_0;
...
chi_a_sim_High = inv(eye(3)/chi_m + N_simulation_High);

%% N simulation Low
load('N_simulation_Low.mat')
chi_a_sim_Low = inv(eye(3)/chi_m + N_simulation_Low);

%% N measurements High
r_sat_ax = 0.9067; %Saturation Point
r_sat_rad = 0.9177;
...
N_high = inv(chi_a_High)- eye(3)/chi_m;
...
N_low = inv(chi_a_Low)- eye(3)/chi_m;
