function [M_sol_handle, M_sol_av] = Magnetisation_Helix(n_helix,B_app,chi_m,N,shape_function)
...
elseif(strcmp(shape_function,'fourier'))
    ...
    for n = 1 : (N-1)/2
        f_shape(2*n-1) = sin(2*pi*i*n*s/(2*pi*k_helix));
        f_shape(2*n) = cos(2*pi*i*n*s/(2*pi*k_helix));
    end
    f_shape(N) = 1;
...
if(strcmp(shape_function,'polynomials'))
    disp(['Calculating Approximation of order N = ' num2str(N)])
...
close all
M_sol(s) = M_pack(s)*PHI;
M_sol_plot = M_sol(s);
M_sol_handle = matlabFunction(M_sol(s),'vars',s)

M_sol_av = 1/(D_helix*pi*k_helix)*integral(matlabFunction(M_sol(s),'vars',s),s_min,s_max,'ArrayValued',true)

figure(1)
ezplot(M_sol_plot(1),[s_min,s_max])
hold on
ezplot(M_sol_plot(2),[s_min,s_max])
ezplot(M_sol_plot(3),[s_min,s_max])
hold off
legend('M_x(s)','M_y(s)','M_z(s)')
title('Magnetization M(s)')
axis tight
grid on

%% Plotting of Magnetization
