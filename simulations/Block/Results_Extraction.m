...
%% REMANENT M
model = mphload('Block Saturated (Remanent M).mph')
%mphnavigator
%info = mphsolinfo(model)
[H_x, H_y, H_z, M_x, M_y, M_z, M_app, n_helix_vec] = mphmean(model,{'mfnc.Hx', 'mfnc.Hy', 'mfnc.Hz', 'mfnc.Mx', 'mfnc.My', 'mfnc.Mz', 'M_app', 'n_helix'},'volume','selection',2,'outersolnum',1);
% solnum choses the innner parametric sweep solution
H = [H_x' H_y' H_z']';
M = [M_x' M_y' M_z']';
H_app = zeros(3,3);
...
SimulationM.('S1').Geometry = 'Rectangle';

save('SimulationM.mat','SimulationM')

%% APPLIED B FIELD
model = mphload('Block Saturated (Applied B).mph')
...
SimulationB.('S1').Geometry = 'Rectangle';
